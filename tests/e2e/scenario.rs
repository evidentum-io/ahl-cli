...
///
/// The profile digest is recomputed over the released `ahl-adaptor-atl-v1` bytes the pilot
/// holds and pinned here. Adaptor §14 makes release a precondition for pinning, and that
/// precondition is met: the document is a released, immutable artifact, shipped verbatim as
/// `ahl_core::ATL_PROFILE_DOCUMENT`. The policy still governs a scratch log rather than a
/// production corpus — that is what makes it a test policy, not the pin.
pub fn policy_file(
    dir: &std::path::Path,
    genesis: &Genesis,
    keys: &Keys,
    ...
    let text = format!(
        "# Generated by the AHL end-to-end pilot. The adaptor profile digest below pins the\n\
         # RELEASED `ahl-adaptor-atl-v1` artifact, shipped verbatim as\n\
         # `ahl_core::ATL_PROFILE_DOCUMENT` and recomputed from the held bytes on every run.\n\
